...
class TestAst(unittest.TestCase):
    def setUp(self):
        self.path = "testcode"
        self.function_dir = path.join(self.path, "functions")
        self.classes_dir = path.join(self.path, "classes")
        self.functions_file = path.join(self.function_dir, "functions.py")
        self.classes_file = path.join(self.classes_dir, "classes.py")
        if not path.exists(self.path):
            makedirs(self.path)
        if not path.exists(self.function_dir):
            makedirs(self.function_dir)
        if not path.exists(self.classes_dir):
            makedirs(self.classes_dir)
